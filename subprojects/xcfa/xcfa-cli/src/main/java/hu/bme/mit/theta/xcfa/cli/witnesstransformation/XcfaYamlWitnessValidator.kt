...
class XcfaYamlWitnessValidator(program: XCFA, witness: XCFA, solver: Solver) {
    ...
    private data class XcfaControl(val location: ControlLocation) : State {
    ...
    > {
        ...
        return builder.build(
            nextSideFunction = { state: ExprMultiState<XcfaControl, XcfaControl, DataState> ->
                // Define alternation strategy between program and witness
                // MultiState getLeftSate() getRightState()...
                when {
                    state.sourceSide == MultiSide.RIGHT -> MultiSide.LEFT
                    state.programState.control.location.isError -> MultiSide.RIGHT
                    else -> MultiSide.BOTH
                }
            },
