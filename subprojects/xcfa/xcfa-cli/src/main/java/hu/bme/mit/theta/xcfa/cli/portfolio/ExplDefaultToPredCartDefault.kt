...
fun explDefaultToPredCartDefault(
    xcfa: XCFA,
    mcm: MCM,
    ...
    val endNodeConfig = baseConfig2.copy(
        backendConfig = baseConfig2.backendConfig.copy(
            inProcess = true,
            specConfig = (baseConfig2.backendConfig.specConfig as CegarConfig).copy(
                refinerConfig = (baseConfig2.backendConfig.specConfig as CegarConfig).refinerConfig.copy(
                    exprSplitter = WHOLE
                )
            )
        ),
        outputConfig = baseConfig2.outputConfig.copy(
            resultFolder =  baseConfig2.outputConfig.resultFolder.resolve("PredCartDefault"),
        )
    )
