...
private fun frontend(
  ...
  val loadPartialResultFromFileIfItExits = { xcfa: XCFA -> 
    config.inputConfig.partialResult?.let { partialResultFile ->
      LocationInvariants.fromFile(partialResultFile, getGson(xcfa), Logger.instance)?.let { loadedInvariants ->
  ...
  var xcfa = getXcfa(config, parseContext)
  loadPartialResultFromFileIfItExits(xcfa);
  xcfa = addPartialResultToXcfa(xcfa, parseContext, currentPartialResult)
  ...
  val result =
    if (config.backendConfig.backend == Backend.NONE) {
      ...
      if (
        ...
        val checker = getChecker(xcfa, mcm, config, parseContext)
        ...
        Logger.result(outResult.toString() + "\n")
...
private fun cleanUp(config: XcfaConfig<*, *>) {
