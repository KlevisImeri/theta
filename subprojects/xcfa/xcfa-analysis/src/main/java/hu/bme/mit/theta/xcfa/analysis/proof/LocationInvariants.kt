...
data class LocationInvariants(
  ...
  fun toJsonFile(file: File, gson: Gson) {
    try {
      val jsonString = gson.toJson(this)
      file.writeText(jsonString)
      Logger.info("[INFO] Successfully wrote LocationInvariants to ${file.absolutePath}\n") 
    } catch (e: Exception) {
  ...
  companion object {
    fun fromFile(file: File, gson: Gson): LocationInvariants? {
      if (!file.exists() || !file.isFile) {
        return null
      }
      if (file.length() == 0L) {
        Logger.info("LocationInvariants ${file.absolutePath} is empty!\n")
      }
      ...
      return try {
        ...
        Logger.info("Successfully loaded LocationInvariants from ${file.absolutePath}\n")
        if(invariants.isEmpty()) {
          Logger.info(yellow("[WARN] LocationInvariants are empty map. Returning null!\n"))
          null
        } else {
          invariants
        }
