...
var timeoutSecDefault = 900*1000L; //sv-comp max time usually
...
fun PredCart(timeoutSec: Long = timeoutSecDefault, pRes: Boolean = false) {
  Cegar(
    domain = PRED_CART,
    pruneStrategy = LAZY,
    timeoutMs = timeoutMs,
    pRes = pRes, 
    ExprSplitterOptions = CONJUNCTS, 
    maxEnum = 2
  );
}

fun Expl(timeoutMs: Long = timeoutSecDefault, pRes: Boolean = false) {
  Cegar(
    domain = EXPL,
    pruneStrategy = FULL,
    timeoutMs = timeoutMs,
...
fun KInd(timeoutMs: Long = timeoutSecDefault, pRes: Boolean = false) {
...
fun Cegar(
      ...
      timeoutMs: Long = timeoutSecDefault,
